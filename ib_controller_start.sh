...
TWS_PATH=/usr/src/ib_controller/tws


#   The folder containing the IBController files:

IBC_PATH=/usr/src/ib_controller/target/IBController
...
IBC_INI=/usr/src/ib_controller/test.ini

#   now launch IBController

"$IBC_PATH/Scripts/IBController.sh" $TWS_MAJOR_VRSN -g "--tws-path=$TWS_PATH" "--ibc-path=$IBC_PATH" "--ibc-ini=$IBC_INI" --user=$TWSUSERID --pw=$TWSPASSWORD
